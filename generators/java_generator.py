...
class JavaCodeGenerator(CodeGenerator):
    ...
    def generate_code(self):
        ...
        try:
            for _, _class in self.__syntax_tree.items():
                ...
                self.get_abstract_methods(_class['relationships']['implements'], _class['relationships']['extends'], abstract_methods)
                abstract_methods.reverse() # reverse the list so that the abstract methods are in the correct order
                abstract_methods = [m for i, m in enumerate(abstract_methods) if m not in abstract_methods[:i]] # remove duplicate abstract methods
                
                associations = self.get_associations(_class['relationships'])
                aggregation_children = self.get_aggregation_children(_class['relationships'])
                aggregation_parents = self.get_aggregation_parents(_class['relationships'])
                composition_children = self.get_composition_children(_class['relationships'])
                composition_parents = self.get_composition_parents(_class['relationships'])
                # join all associations
                all_associations = associations + aggregation_children + aggregation_parents + composition_children + composition_parents
                ...
                if _class['properties'] or _class['relationships']['association'] \
                        or _class['relationships']['aggregationChildren'] or _class['relationships']['aggregationParents'] \
                        or _class['relationships']['compositionChildren'] or _class['relationships']['compositionParents']:
                    file += "\n"
                file += self.generate_methods(_class['methods'], _class['properties'], all_associations, _class['type'], abstract_methods)
                file += "}\n" 
    ...
    def get_associations(self, relationships):
        """
        ...
        for association in relationships['association']:
            associations.append(self.__syntax_tree[association]['name'])

        return associations

    def get_aggregation_children(self, relationships):
        """
        ...
        for aggregation in relationships['aggregationChildren']:
            aggregation_children.append(self.__syntax_tree[aggregation]['name'])

        return aggregation_children

    def get_aggregation_parents(self, relationships):
        """
        ...
        for aggregation in relationships['aggregationParents']:
            aggregation_parents.append(self.__syntax_tree[aggregation]['name'])

        return aggregation_parents

    def get_composition_children(self, relationships):
        """
        ...
        for composition in relationships['compositionChildren']:
            composition_children.append(self.__syntax_tree[composition]['name'])

        return composition_children

    def get_composition_parents(self, relationships):
        """
        ...
        for composition in relationships['compositionParents']:
            composition_parents.append(self.__syntax_tree[composition]['name'])

        return composition_parents

    def generate_methods(self, methods, properties, all_asssociations, class_type, abstract_methods):
        """
        ...
        if class_type == "class" or class_type == "abstract":
            ...
            for association in all_asssociations:
                getter = (f"\tpublic {association} get{association[0].upper() + association[1:]}()"
                          f" {{\n \t\treturn this.{association[0].lower() + association[1:]}; \n\t}}\n");
                methods_string += getter + "\n"
                self.__methods.append(getter)

                setter = (f"\tpublic void set{association[0].upper() + association[1:]}({association} {association[0].lower() + association[1:]})"
        ...
        if class_type == "class":
            for abstract_method in abstract_methods:
                ...
                m = (f"\t{abstract_method['access']} {abstract_method['return_type']} {abstract_method['name']}()"
                     f" {{\n \t\t{comment} \n\t}}\n")
                methods_string += m + "\n"
        ...
        for _, method_value in methods.items():
            if not method_value['parameters']:
                ...
                params = "(" + ", ".join([f"{p['type']} {p['name']}" for p in method_value['parameters'].values()]) + ")"
    ...
    def get_abstract_methods(self, implements, extends, abstract_methods): 
        """
        ...
        for i in implements:
            ...
            methods = list(interface_obj['methods'].values()) # take all methods from interface
            methods.reverse() # reverse the list so that the methods are in the correct order
            abstract_methods += methods
            self.get_abstract_methods(interface_obj['relationships']['extends'], interface_obj['relationships']['extends'], abstract_methods)

        for e in extends:
            class_obj = self.__syntax_tree[e]
            methods = [m for m in class_obj['methods'].values() if m['abstract'] == 'true'] # take only abstract methods from class
            methods.reverse() # reverse the list so that the methods are in the correct order
            abstract_methods += methods
            self.get_abstract_methods(class_obj['relationships']['implements'], class_obj['relationships']['extends'], abstract_methods)
